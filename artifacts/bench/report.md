...
|---|-----------|--------|------|-------|-------|--------------|--------|
| 1 | setup_friction | MEASURED | CAPABILITY | 0.286 | 100.0 | no | Cold `init` + first `plan` completed in 0.29s (target <300s). |
...
| 9 | tui_task_completion | UNAVAILABLE | UNAVAILABLE | — | — | no | TUI review independence is not corroborated. A self-declared 'is_builder:false' is insufficient; supply corroboration.independent_evidence (e.g. a screen recording artifact + a non-builder reviewer identity). Operator attestation is recorded but not credited as a measured score. |
