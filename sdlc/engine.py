...
def deterministic_artifacts_for_gate(plan: RunPlan, gate_id: str, run_dir: Path, *, full_advisory: bool = False) -> tuple[str, list[str], str | None]:
    repo = Path(plan.repo)
    artifacts: list[str] = []
    ...
    if gate_id == "repo_context_env_branch":
        ...
        candidates = _detected_validation_commands(repo, quality_only=True)
        failures: list[str] = []
        if not candidates:
        ...
        content_lines.extend(_advisory_gate_lines(plan, gate_id, run_dir))
        verdict_override = _advisory_gate_verdict(plan, gate_id, run_dir, repo, content_lines)
    else:
...
def _advisory_gate_lines(plan: RunPlan, gate_id: str, run_dir: Path) -> list[str]:
    gate = GATE_DEFINITIONS.get(gate_id)
    lines = [
        ...
        f"Gate: {gate_id}",
        f"Owner: {gate.owner if gate else '<unknown>'}",
        "",
        "This artifact was generated by the SDLC control plane without an external",
        "worker. It is useful advisory evidence, but it is not live/release",
        "clearance unless the gate verdict and release validation say so.",
        "",
    ]
    if gate:
        lines.extend([
        ...
        for artifact in gate.required_artifacts:
            lines.append(f"- {artifact}: {_advisory_artifact_summary(plan, gate_id, artifact)}")
        lines.append("")
    lines.extend(_advisory_gate_specific_lines(plan, gate_id, run_dir))
    return lines


def _advisory_artifact_summary(plan: RunPlan, gate_id: str, artifact: str) -> str:
    if gate_id == "intake_scope":
        return f"Derived from feature request `{plan.feature}` and classifier output."
    if gate_id == "risk_blast_radius":
        return f"Risk={plan.risk_level}; activated_agents={len(plan.agents)}."
    if gate_id == "agent_plan_permissions":
        return "Derived from pipeline owners and active policy; run `sdlc agents plan` for the full matrix."
    if gate_id == "architecture_contracts":
        return "Advisory architecture scaffold; requires worker/human design review for release use."
    if gate_id == "implementation":
        return "Requires a real diff or executed worker evidence."
    if gate_id == "qa_tests_integration_smoke":
        return "Requires local test command evidence."
    if gate_id == "independent_redteam_cross_model":
        return "Requires executed independent red-team workers for high-stakes release readiness."
    return "Deterministic advisory placeholder generated from run context."


def _advisory_gate_specific_lines(plan: RunPlan, gate_id: str, run_dir: Path) -> list[str]:
    classification = plan.classification or {}
    lines: list[str] = []
    if gate_id == "intake_scope":
        lines.extend([
            "## Scope",
            f"- Request: {plan.feature}",
            f"- Risk: {plan.risk_level}",
            f"- UI: {classification.get('has_ui')}",
            f"- Security: {classification.get('has_security')}",
            f"- Infrastructure: {classification.get('has_infra')}",
            "",
        ])
    elif gate_id == "stakeholders_raci":
        lines.extend([
            "## RACI",
            "- Accountable: human release owner for production or residual-risk decisions.",
            "- Responsible: role agents named in the run plan.",
            "- Consulted: security, architecture, QA, and domain specialist roles when activated.",
            "- Informed: operators consuming the final report.",
            "",
        ])
    elif gate_id == "mission_non_goals":
        lines.extend([
            "## Claim Discipline",
            "- Do not claim production-ready, secure, compliant, profitable, or world-class unless release evidence proves it.",
            "- Treat this artifact as planning evidence, not implementation proof.",
            "",
        ])
    elif gate_id == "risk_blast_radius":
        agent_ids = [str(agent.get("id")) for agent in plan.agents]
        lines.extend([
            "## Risk And Blast Radius",
            f"- Risk level: {plan.risk_level}",
            f"- Production rollout allowed: {plan.production_rollout_allowed}",
            f"- Direct main push allowed: {plan.direct_main_push_allowed}",
            "- Activated agents:",
            *(f"  - {agent_id}" for agent_id in agent_ids),
            "",
        ])
    elif gate_id == "data_privacy_secrets":
        lines.extend([
            "## Data And Secret Controls",
            "- No secrets in repo files, prompts, logs, worker outputs, or run artifacts.",
            "- External network and production access remain policy-controlled.",
            "- Prompt-injection and path traversal must be tested for tool/MCP surfaces.",
            "",
        ])
    elif gate_id == "agent_plan_permissions":
        lines.extend([
            "## Agent Ownership",
            "- Run `sdlc agents plan <run-id>` for the machine-readable task plan.",
            "- Implementers may not close their own red-team findings.",
            "- Red-team roles are read-only by default.",
            "",
        ])
    elif gate_id == "architecture_contracts":
        lines.extend([
            "## Architecture Review Targets",
            "- Define source-of-truth contracts before implementation.",
            "- Separate simulation/read-only modes from future live write modes.",
            "- Capture invariants, failure modes, rollback, and observability before release.",
            "",
        ])
    elif gate_id == "ui_architecture_accessibility":
        lines.extend([
            "## UI Review Targets",
            "- Every interactive control needs a testable selector.",
            "- Mobile/read-only states and stale data states must be explicit.",
            "- UI must not imply release readiness when evidence is missing.",
            "",
        ])
    elif gate_id == "threat_model_abuse_cases":
        lines.extend([
            "## Threat Model Seeds",
            "- Prompt injection in evidence, logs, reports, transcripts, and issue text.",
            "- Credential leakage through worker output or artifacts.",
            "- Unauthorized production or broker/exchange writes.",
            "- Forged reports, stale evidence, and replay tampering.",
            "",
        ])
    elif gate_id == "implementation_plan_changeset":
        lines.extend([
            "## Minimal Change-Set Contract",
            "- Implement the smallest vertical slice that satisfies required deliverables.",
            "- Keep generated evidence separate from source authority.",
            "- Add tests before treating changed behavior as complete.",
            "",
        ])
    elif gate_id == "observability_runbooks":
        lines.extend([
            "## Operations Checklist",
            "- Logs must identify run id, artifact id, and decision id where applicable.",
            "- Runbook must include rollback, smoke validation, and residual-risk owner.",
            "- No production rollout is authorized by this advisory artifact.",
            "",
        ])
    elif gate_id == "evidence_traceability_attestations":
        artifacts = sorted(str(path.relative_to(run_dir)) for path in run_dir.rglob("*") if path.is_file() and path.name != "events.lock")[:80]
        lines.extend(["## Artifact Index Sample", *(f"- {artifact}" for artifact in artifacts), ""])
    return lines


def _advisory_gate_verdict(plan: RunPlan, gate_id: str, run_dir: Path, repo: Path, content_lines: list[str]) -> str | None:
    if gate_id in ADVISORY_GO_GATE_IDS:
        return "GO"
    if gate_id == "implementation":
        result = run_cmd(["git", "diff", "--stat"], repo)
        content_lines.extend(["## Implementation Diff", "$ git diff --stat", f"returncode: {result['returncode']}", result["stdout"] or "<no uncommitted diff>"])
        if result["returncode"] == 0 and result["stdout"].strip():
            return "GO"
        content_lines.append("No implementation diff detected; development cannot be marked complete.")
        return "NO_GO"
    if gate_id == "qa_tests_integration_smoke":
        commands = _detected_validation_commands(repo)
        failures: list[str] = []
        if not commands:
            content_lines.append("No local test command detected.")
            return "NO_GO"
        for cmd in commands[:3]:
            result = run_cmd(cmd, repo, timeout=180)
            content_lines.extend([f"$ {' '.join(cmd)}", f"returncode: {result['returncode']}", result["stdout"] or result["stderr"] or "<empty>"])
            if result["returncode"] != 0:
                failures.append(" ".join(cmd))
        if failures:
            content_lines.extend(["Failing QA commands:", *(f"- {item}" for item in failures)])
            return "NO_GO"
        return "GO"
    if gate_id == "implementer_self_review":
        content_lines.append("No implementer self-review evidence was produced by a worker or human.")
        return "NO_GO"
    if gate_id == "independent_redteam_cross_model":
        content_lines.append("Independent executed red-team evidence is required; deterministic advisory review is not enough.")
        return "NO_GO"
    if gate_id == "critical_high_fix_loop":
        findings = read_json(run_dir / "findings.json", [])
        open_high = [
            item for item in findings
            if str(item.get("severity", "")).upper() in {"CRITICAL", "HIGH"} and str(item.get("status", "OPEN")).upper() not in {"CLOSED", "ACCEPTED"}
        ]
        content_lines.append(f"Open CRITICAL/HIGH findings: {len(open_high)}")
        return "NO_GO" if open_high or plan.risk_level in {"HIGH", "EXTREME"} else "GO"
    if gate_id in {"evidence_traceability_attestations", "commit_branch_pr_ci", "final_report_reaudit"}:
        content_lines.append("Release-grade provenance/final-report evidence is not complete in the no-worker advisory pass.")
        return "NO_GO"
    return "NO_GO"
...
def run_dry_gates(store: RunStore, run_id: str, *, full_advisory: bool = False) -> RunPlan:
    ...
    for gate in sorted(plan.gates, key=lambda item: item.order):
        ...
        if blockers and not full_advisory:
            gate.state = "BLOCKED"
            gate.verdict = "NO_GO"
            ...
            continue
        if blockers and full_advisory:
            ledger.event("gate.advisory_prerequisites_unresolved", gate=gate.id, blockers=blockers)
        gate.state = "RUNNING"
        ledger.event("gate.started", gate=gate.id, owner=gate.owner)
        _content, artifacts, verdict_override = deterministic_artifacts_for_gate(plan, gate.id, run_dir, full_advisory=full_advisory)
        ...
        if gate.id not in DRY_GO_GATE_IDS and not full_advisory:
            gate.state = "FIX_REQUIRED" if gate.id != "independent_redteam_cross_model" else "NO_GO"
            gate.verdict = "NO_GO"
            gate.notes = "Agent or human evidence required; dry-run placeholder/advisory artifact cannot mark this gate GO."
        ...
        elif gate.id not in DRY_GO_GATE_IDS and full_advisory:
            if verdict_override == "GO":
                gate.state = "GO"
                gate.verdict = "GO"
                gate.notes = "Full advisory artifact captured by SDLC without worker execution. Release validation still applies."
                ledger.event("gate.completed", gate=gate.id, verdict="GO", evidence=artifacts, advisory=True)
            else:
                gate.state = "NO_GO" if gate.id == "independent_redteam_cross_model" else "FIX_REQUIRED"
                gate.verdict = "NO_GO"
                gate.notes = "Full advisory pass generated evidence, but this gate still needs implementation, worker, human, or release-grade proof before GO."
                ledger.event("gate.no_go", gate=gate.id, evidence=artifacts, notes=gate.notes, advisory=True)
        elif verdict_override == "NO_GO":
            gate.state = "NO_GO"
            gate.verdict = "NO_GO"
            gate.notes = _content or "Deterministic gate evidence returned NO_GO."
            invalidated = [] if full_advisory else invalidate_downstream_gates(plan, gate.order, f"Blocked because prerequisite gate {gate.id} is NO_GO.")
            ledger.event("gate.no_go", gate=gate.id, evidence=artifacts, notes=gate.notes)
            if invalidated:
                ledger.event("gate.downstream_invalidated", gate=gate.id, invalidated=invalidated)
        else:
