...
- **Honest headline:** the benchmark headline now averages only CORPUS dimensions and is
  ...
  dimensions are excluded (this is why the headline dropped from the earlier inflated 88
  to the honest, corpus-relative 87.5 on the committed reference corpus).
...
- **Release signature verified:** `cosign verify-blob` against the `KEYS.md` identity on
  the real `v0.2.0` assets returns **Verified OK**.
...
`v0.2.0` Sigstore signature verifies against the KEYS.md identity.
...
- Committed measurement (`artifacts/bench/comparative.json`, reference corpus):
  **median 4x** across **4 runs** (`proven_100x: false`). The factor is
  **corpus-relative** — on a larger/older run corpus it ran higher (low-double-digit×),
  but the only figure we publish is the one reproducible from the committed artifact.
- **100x is not proven** on this metric.
- The measurement is conservative — it under-counts manual effort (it excludes the
  ...
  so the true advantage is somewhat higher than the reported factor but nowhere near 100x.
...
**Bottom line:** the honest, defensible claim is "a measured ~4x fewer inspection steps
(corpus-relative) to find release blockers, plus capabilities a generic coding agent
lacks entirely" — not "100x better than Claude Code."
